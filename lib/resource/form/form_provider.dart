...
class FormProvider {
  ...
  Future<String> checkExistAdoptionRegistrationForm(String petProfileId) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var jwtToken = sharedPreferences.getString('token');
  ...
  Future<FinderForm> getFinderFormById(String finderId) async {
    final response = await http.get(
      ApiUrl.getFinderFormById + finderId,
      headers: {
    ...
    if (response.statusCode == 200) {
      return FinderForm(json.decode(response.body));
    } else {
      print('Failed to load post ${response.statusCode}');
    }
    return null;
  ...
  Future<AdoptionRegisForm> getAdoptRegistrationFormById(String adoptionRegistrationId) async {
    final response = await http.get(
      ApiUrl.getAdoptRegistrationFormById + adoptionRegistrationId,
      headers: {
        "Accept": "application/json",
        "Content-Type": "application/json; charset=UTF-8",
      },
    ...
    if (response.statusCode == 200) {
      return AdoptionRegisForm(json.decode(response.body));
    } else {
      print('Failed to load post ${response.statusCode}');
    }
    return null;
  ...
  Future<bool> cancelAdoptionRegistrationForm(
      String adoptionRegistrationFormId, String reason) async {
    SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
    var jwtToken = sharedPreferences.getString('token');
